...
export function Cotizaciones() {
  ...
  const generarBoleta=async cotizacion=>{
    try {
      setError('');setCorrecto('')
      const respuesta=await Solicitar(`/cotizaciones/${cotizacion.id}/boleta`,{method:'POST',body:'{}'})
      setCorrecto(`Boleta ${respuesta.numero} generada`);await cargar()
  ...
  return <div className="pagina">
    ...
    <div className="tabla"><table>
      ...
      <tbody>{cotizaciones.map(c=><tr key={c.id}>
        ...
        <td><div className="accionesfila vertical"><button className="secundario" onClick={()=>{setSeleccion(c);setAccion('detalle')}}>Ver detalle</button>{c.comprobanteen&&['Administrador','AsesorVentas'].includes(usuario.rol)&&<button className="secundario" onClick={()=>Descargar(`/cotizaciones/${c.id}/comprobante`,c.comprobantenombre||'comprobante')}>Comprobante</button>}{c.estado==='Entregada'&&!c.boletacodigo&&<button className="secundario" onClick={()=>generarBoleta(c)}>Generar boleta</button>}{c.boletacodigo&&<><button className="secundario" onClick={()=>Descargar(`/cotizaciones/${c.id}/boleta/pdf`,`${c.boletanumero}.pdf`)}>Descargar boleta</button><a className="secundario botonverificacion" href={`/?verificarboleta=${c.boletacodigo}`} target="_blank" rel="noreferrer">Verificar boleta</a></>}{acciones(c).map(([id,nombre])=><button key={id} className={id.includes('rechazar')?'secundario peligrotexto':'secundario'} onClick={()=>abrir(c,id)}>{nombre}</button>)}<button className="secundario" onClick={()=>verHistorial(c)}>Historial</button></div></td>
      </tr>)}</tbody>
    </table></div>
