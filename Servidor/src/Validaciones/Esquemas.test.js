...
test('rechaza telefono que no empieza en 9',()=>assert.equal(EsquemaCotizacion.safeParse({...CotizacionValida,telefono:'887654321'}).success,false))
test('rechaza telefono con menos de 9 digitos',()=>assert.equal(EsquemaCotizacion.safeParse({...CotizacionValida,telefono:'98765432'}).success,false))
test('rechaza correo con parte local muy corta',()=>assert.equal(EsquemaCotizacion.safeParse({...CotizacionValida,correo:'ab@empresa.pe'}).success,false))
test('rechaza correo con dominio mayor a 15 caracteres',()=>assert.equal(EsquemaCotizacion.safeParse({...CotizacionValida,correo:'ventas@unaempresagrande.com'}).success,false))
test('rechaza correo con ñ o tildes',()=>assert.equal(EsquemaCotizacion.safeParse({...CotizacionValida,correo:'ventañ@empresa.pe'}).success,false))
test('rechaza correo con caracteres especiales',()=>assert.equal(EsquemaCotizacion.safeParse({...CotizacionValida,correo:'ventas+1@empresa.pe'}).success,false))
